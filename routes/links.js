...
router.post('/create', async (req, res, next) => {
  try {
    // accept either field names (destination or url) and either custom or customCode
    let destination = (req.body.destination || req.body.url || '').toString().trim();
    let custom = (req.body.custom || req.body.customCode || '').toString().trim();
...
router.get('/:code', async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.error('Failed to increment clicks:', e && e.message ? e.message : e);
    }
